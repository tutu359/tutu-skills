...
2. Ticket 执行顺序及依赖关系；
3. 当前仓库根目录、当前分支、启动时基准 commit 和工作区状态；
4. 每个 worktree 的唯一路径和安全分支名；
...
1. 锁定基准 commit。第一个 Ticket 使用流程开始时锁定的 commit；每个后续 Ticket 使用前一个 Ticket 的完成 commit。
2. 在主仓库同级目录创建独立 worktree 和分支。创建失败时停止，不得退回当前工作区。
...
6. A 返回完成报告后，主代理只核验编排所需事实：报告对应 Ticket、worktree 和分支存在、完成 commit 位于预期基线之上、主工作区未被修改，以及 A 已给出实现检查结果。主代理不重新执行实现细节，也不审查后代报告。
7. 将 A 的完成或阻塞报告摘要追加到项目根目录的 `ticket-flow-record.md`。记录文件不纳入 Git。
8. 只有完成报告核验通过后，才创建下一个 Ticket 的 worktree。

## Worktree、分支和 commit

- 每个 Ticket 使用独立 worktree 和独立分支；worktree、分支和 commit 全部保留。
- 分支名必须包含真实 Ticket 编号和短名称，例如 `ticket/ABC-123-add-refund-flow`。
- worktree 路径必须明确、唯一，并记录在报告中，例如 `<repo-root>/../worktrees/ticket-ABC-123-add-refund-flow`。
...
- 后代不得直接向主代理发送消息或报告。若运行环境无法可靠保证该路由，A 不得创建后代子代理。
...
任一 Ticket 缺失、顺序不明、worktree 创建失败、A 第 5 次调用仍失败、A 返回阻塞、完成报告核验失败或主工作区发生变化，立即停止后续 Ticket，并在记录文件中写入阻塞报告，等待用户决定。

全部 Ticket 完成后：

1. 保留全部 worktree、分支和 commit，不合并任何变更；
2. 在记录文件中追加总执行状态；
